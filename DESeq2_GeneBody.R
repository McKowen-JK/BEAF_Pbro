...
write.csv(as.data.frame(res.BF_LZ_DESeq), file=paste(c(OutputDirectory,'BF_LZ_GeneBody_DESeq2.csv'), collapse = ''))
write.csv(BF_LZ_DESeq, '/home/chart/PROseq2/DESeq/DESeq_gene_lists/bf_lz_gb_counts')
...
write.csv(as.data.frame(res.PB_LZ_DESeq), file=paste(c(OutputDirectory,'PB_LZ_GeneBody_DESeq2.csv'), collapse = ''))
write.csv(PB_LZ_DESeq, '/home/chart/PROseq2/DESeq/DESeq_gene_lists/pb_lz_gb_counts')
...
write.csv(as.data.frame(res.BF_PB_DESeq), file=paste(c(OutputDirectory,'BF_PB_GeneBody_DESeq2.csv'), collapse = ''))


plotMA(DESeq.BF_PB_DESeq)
title(main='BEAF RNAi vs PBro RNAi', cex=1.5)

#################################################Now do DESeq2 to look for changes between WT and Apt cells Before Copper treatment


S2_vs_Apt_DESeq_beforeCu = output[,c(13:14, 9:10)]
rownames(S2_vs_Apt_DESeq_beforeCu) <- output[,4]

coldata_S2 = data.frame(row.names=colnames(S2_vs_Apt_DESeq_beforeCu),condition=as.factor(c(rep("untreated",2),rep("treated",2))))    #I don't understand this... just copy from Fabs

dds.S2_vs_Apt_DESeq_beforeCu <- DESeqDataSetFromMatrix(
  countData = S2_vs_Apt_DESeq_beforeCu,
  colData = coldata_S2,
  design = ~ condition)   #build the DESeq matrix.  Uses the metadata from coldata and the counts from S2_vs_Apt_DESeq_beforeCu.  Design lets it know that the condition column in the coldata frame indicate which are the treated samples (points to the column in coldata that has the options for treatment... I only have teated or untreated, but there could be multiple variables like time, concentration, and temperature, etc)

dds.S2_vs_Apt_DESeq_beforeCu$condition <- relevel(dds.S2_vs_Apt_DESeq_beforeCu$condition, "untreated")  #makes sure that the untreated samples are first (so that fold change is right when DESeq calculates it).  Probably unnecessary as it is already in this order.

DESeq.S2_vs_Apt_DESeq_beforeCu=DESeq(dds.S2_vs_Apt_DESeq_beforeCu)
res.S2_vs_Apt_DESeq_beforeCu = results(DESeq.S2_vs_Apt_DESeq_beforeCu)

write.csv(as.data.frame(res.S2_vs_Apt_DESeq_beforeCu), file=paste(c(OutputDirectory,'S2_vs_Apt_neg_GeneBody_DESeq2.csv'), collapse = ''))


plotMA(DESeq.S2_vs_Apt_DESeq_beforeCu)
